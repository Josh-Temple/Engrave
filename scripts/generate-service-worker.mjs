...
self.addEventListener('activate', event => event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith('engrave-') && key !== CACHE).map(key => caches.delete(key)))).then(() => self.clients.claim())));
