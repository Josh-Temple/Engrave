...
`npm run build` generates a service worker from the actual Vite `dist` file list. Its deterministic cache generation fingerprint covers each precached path and its content hash (excluding `sw.js`), so changes to fixed-name files such as the manifest or icon also trigger an update. It precaches hashed JavaScript/CSS plus HTML, manifest, icon, and KaTeX fonts, and uses a network-first navigation response with an offline HTML fallback. An installed update waits until the user selects the small **Update now** notice. Activation retains the previous cache generation, reducing old-tab breakage while the new client takes control; offline lookup checks the current generation before that retained previous generation.

Raw HTML processing is disabled. Consecutive body segments without readings are reconstructed before React Markdown processes them with `remark-math` and `rehype-katex`, allowing Quick Add word, character, and smart segmentation to preserve complete Markdown/math syntax. Ruby/pinyin segments and explicit line breaks are reconstruction boundaries. Study cloze selection is separate from that display reconstruction: ordinary text keeps its original segment-level progression, while complete math, emphasis, inline-code, and link constructs are atomic so their syntax is never partly blanked. Whitespace, punctuation, and line breaks are never selected; level 4 and Practice Mode blank every selectable unit. A segment with a reading is deliberately treated as plain text and placed in trusted React `ruby`/`rt` elements; Markdown and LaTeX are not interpreted inside either its text or reading. Neither `rehypeRaw` nor `dangerouslySetInnerHTML` is used. Notes are always React text nodes. Consequently no user-provided HTML elements or attributes are allowed: `script`, `iframe`, `object`, `embed`, `style`, `svg`, event attributes, and `javascript:` URLs cannot become executable markup. Boundary whitespace (including whitespace-only segments), explicit newlines, and blank lines are preserved exactly.
